sig2map_HE	<- function( sig2s, gtype, etype, K0, disc_Z ){
	if( gtype == 'hom' & etype == 'hom' ){
		sig2g	<- sig2s[1]
		sig2e	<- sig2s[2]
		form	<- list( as.formula("~(x1)/(x1+x2)"))
	} else if( gtype == 'iid' & disc_Z ){
      sig2g	<- sig2s[1:2]
      sig2e	<- sig2s[3]
      form	<- lapply( 1:2, function(k) as.formula( paste0( '~x', k, '/(x1+x2+x3)' ) ) )
	} else if( gtype == 'iid' & !disc_Z ){
    if( etype != 'iid' ) stop('With non-discrete Z, etype should be iid when gtype is iid')
		sig2g	<- sig2s[1:2]
		sig2e	<- sig2s[3:4]
		form	<- lapply( 1:2, function(k) as.formula( paste0( '~x', k, '/(x1+x2+x3+x4)' ) ) )
	...
	#	}
  } else if( gtype == 'free' & etype == 'free' & disc_Z ){
		sig2g		<- sig2s[1:(K0+1)]
	...
	if( gtype == 'hom' & etype == 'hom' ){
		...
		names(h2)	<- c( 'hom', 'het' )
  } else if( gtype == 'free' & disc_Z ){
    h2					<- (sig2g[1] + sig2g[-1])/(sig2g[1] + sig2g[-1] + sig2e)
    names(h2)<- c( 'sig2g_hom', paste0( 'sig2g_', 1:K0 ) )
	} else if( gtype == 'free' & !disc_Z ){
    h2					<- (sig2g[1] + sig2g[-1])/(sig2g[1] + sig2g[-1] + sig2e)
    names(h2)<- c( 'sig2g_hom', paste0( 'sig2g_', 1:K0 ) )
  }
